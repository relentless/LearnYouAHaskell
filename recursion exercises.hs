...
power :: Int -> Int -> Int
power _ y | y < 0 = error "I only recognise positive powers!"
power _ 0 = 1
...
stepReverseSign num step = (abs num + step) * (- signum num) -- Not sure what this was about.. An exercise in understanding requirements?
-- Ah you probably need it for the next one
...
{- Lets calculate pi.
 ...
 -- Bit different
piCalc :: (Fractional a, Ord a, Integral b) => b -> a
piCalc iterations = 
    piCalc' 1.0 0.0 iterations 0
    where
        piCalc' denominator pi iterations calls 
                | calls >= iterations = pi
                | otherwise = piCalc' (stepReverseSign denominator 2) (pi + (4.0/denominator)) iterations (calls+1)