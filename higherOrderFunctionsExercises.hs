...
hoSumSquares = higherOrderSum (\x -> x * x)
...
higherOrderSequenceApplication valueFunc operatorFunc start end base = 
    sequenceApplication start
    where
        sequenceApplication current 
            | current > end = base
            | otherwise = (valueFunc current) `operatorFunc` (sequenceApplication $ increment current)

doThing = higherOrderSequenceApplication (+ 1) (+) 1 3 0 -- add 1 and sum

doThing2 = higherOrderSequenceApplication (\x -> x * x) (*) 1 3 1 -- square and multiply

-- Define a factorial method using the higherOrderSequenceAppliction
hoFactorial :: Int -> Int
hoFactorial n = higherOrderSequenceApplication (\x -> x) (*) 1 n 1